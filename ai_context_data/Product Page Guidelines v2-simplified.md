...
All images except testimonial headshots must be illustration-style assets or branded photography from the media library. Search with descriptive queries including the product name (e.g., "FinDrop virtual cards illustration", "flight booking illustration").
...
**Alternating pattern:** Alternate image position (left/right) between consecutive sections. One section may use `accent` background with padding `64` to break up the rhythm.
...
Section overrides: padding `64`, background color `muted`. Grid: `100` for 1 testimonial, `50-50` for 2.

Card style: inverted, center-aligned. Include headshot from media library when available.
...
Section overrides: margin bottom `64`, padding `64`, background color `muted`.
