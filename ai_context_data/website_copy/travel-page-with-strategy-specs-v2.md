...
**`main_slot`** → 2× `sdc.byte_theme.group`
...
Contains: `sdc.byte_theme.image` — Custom illustration. A booking flow showing a hotel selection screen with a policy check overlay. One option flagged as over-limit, another marked compliant.

**Group 2 (text content, right):**
...
| Child | Component | Key Props |
|-------|-----------|-----------|
| Eyebrow | `text` | "POLICY ENFORCEMENT", `text_size`: `text-sm`, `text_color`: `primary` |
...
**`main_slot`** → 2× `sdc.byte_theme.group` (note: image/text positions swap — text left, image right)

**Group 1 (text content, left):**
...
| Child | Component | Key Props |
|-------|-----------|-----------|
| Eyebrow | `text` | "VIRTUAL CARDS", `text_size`: `text-sm`, `text_color`: `primary` |
...
Contains: `sdc.byte_theme.image` — Custom illustration. A virtual card labeled with a trip name and destination, showing a spending limit bar and a list of tagged transactions.
...
**`main_slot`** → 2× `sdc.byte_theme.group` (image left, text right)
...
Contains: `sdc.byte_theme.image` — Custom illustration. FinDrop Copilot mobile chat interface showing a traveler asking about a hotel rate in Tokyo, with Copilot responding with the per-night cap and a "book within policy" button.
...
| Child | Component | Key Props |
|-------|-----------|-----------|
| Eyebrow | `text` | "COPILOT", `text_size`: `text-sm`, `text_color`: `primary` |
...
Both cards share: `align`: `center`, `style`: `inverted`
...
**`main_slot`** → 2× items: `sdc.byte_theme.group` (text content, left) + image or illustration (right)
...
**Right column:** Custom illustration from the Travel media library — travel/finance imagery conveying the platform story.
