...
| `accent` | `#DBEAFE` | Light blue contrast | The single most important section on the page (max one) |
...
2. **`accent` — maximum one per page.** Use it on the single most important section — the one thing you want the reader to notice. This could be a feature deep-dive, the testimonials, or another section the author chooses. When `accent` is used, text inside must use `inverted` text color (`#020618`) and buttons must use non-inverted variants.
3. **`muted` — Final CTA only.** The bottom CTA band always uses `muted`. It's the only section that uses `muted`. Do not use `muted` for testimonials, features, or other sections.
...
**Background variation:** The author may designate one deep-dive section as the page's single `accent` section — if so, set `background_color`: `accent`, `padding_block_start`: `64`, `padding_block_end`: `64`. Use `inverted` text color on all child text and headings. Only do this if the `accent` slot is not already used by another section (e.g., testimonials). Maximum one `accent` section per page, total.
...
No `background_color` by default (sits on the dark page background). The author may designate this as the page's one `accent` section — if so, set `background_color`: `accent`, `padding_block_start`: `64`, `padding_block_end`: `64`, and use `inverted` text color on child components.
...
If this section uses `accent` background, switch card `style` to `inverted`.
...
- [ ] Background colors only where allowed (`muted` on Final CTA only, max one `accent` section on the whole page)?
- [ ] No card-icon backgrounds on sections that already have a background color?
